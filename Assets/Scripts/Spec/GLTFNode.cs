...
namespace Siccity.GLTFUtility
{
		...
		public class ImportTask : Importer.ImportTask<ImportResult[]>
		{
			...
			public override IEnumerator OnCoroutine(Action<float> onProgress = null)
			{
				...
				for (int i = 0; i < Result.Length; i++)
				{
					...
					{
						Result[i].transform.gameObject.GetComponent<GLTFTypeInfo>().m_extrasA = nodes[i].extras;
					}
				}
				// Set up hierarchy
				for (int i = 0; i < Result.Length; i++)
	...
	[Serializable]
	public class extrasA
	{
		public int ElementID;
		public string UniqueId;
		public List<ParametersA> Parameters;
	}
	[Serializable]
	public class ParametersA
	{
		public string GroupName;
		public List<ParametersB> Parameters;
	}
	[Serializable]
